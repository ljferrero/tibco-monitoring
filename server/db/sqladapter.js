...
const moutArray = require('mout/array');
const moutLang = require('mout/lang');
const moutObj = require('mout/object');
const moutString = require('mout/string');
const DSUtils = require('js-data').DSUtils;
...
function getTable(resourceConfig) {
    return resourceConfig.table || moutString.underscore(resourceConfig.name);
}
    ...
    while (fieldParts.length >= 2) {
        ...
        if (relation) {
            ...
            if (relation.type === 'belongsTo' || relation.type === 'hasOne') {
                ...
                let subQueryOptions = moutObj.deepMixIn({}, options, { query: knex(this.defaults).select(`${subQueryTable}.*`).from(subQueryTable) });
...
function loadWithRelations(items, resourceConfig, options) {
    ...
    if (resourceConfig.relationList) {
        resourceConfig.relationList.forEach(def => {
            ...
            if (moutArray.contains(options.with, relationName)) {
                containedName = relationName;
            } else if (moutArray.contains(options.with, def.localField)) {
                containedName = def.localField;
            } else {
                return;
            }

            let __options = moutObj.deepMixIn({}, options.orig ? options.orig() : options);
            ...
            if ((def.type === 'hasOne' || def.type === 'hasMany') && def.foreignKey) {
                ...
                task = this.findAll(resourceConfig.getResource(relationName), {
                    where: {
                        [relationDef.idAttribute]: {
                            'in': filter(moutArray.unique(localKeys), x => x)
                        }
                    }
                }, __options).then(relatedItems => {
                    if (instance) {
                        ...
                        items.forEach(item => {
                            let itemKeys = item[def.localKeys] || [];
                            let attached = relatedItems.filter(ri => itemKeys && moutArray.contains(itemKeys, ri[relationDef.idAttribute]));
                            item[def.localField] = attached;
                        })
                    }
                ...
                if (instance) {
                    let id = moutObj.get(instance, def.localKey);
                    if (id) {
                        task = this.findAll(resourceConfig.getResource(relationName), {
                            where: {
                    ...
                    let ids = items.map(item => moutObj.get(item, def.localKey)).filter(x => x);
                    if (ids.length) {
                        task = this.findAll(resourceConfig.getResource(relationName), {
                            where: {
...
class DSSqlAdapter {
    ...
    constructor(options) {
        ...
        moutObj.deepMixIn(this.defaults, options);
    }
    ...
    find(resourceConfig, id, options) {
        ...
        return query
            .select(`${table}.*`)
            .from(table)
            .where(`${table}.${resourceConfig.idAttribute}`, moutLang.toString(id))
            .then(rows => {
                if (!rows.length) {
    ...
    create(resourceConfig, attrs, options) {
        attrs = removeCircular(moutObj.omit(attrs, resourceConfig.relationFields || []));
        let query = options && options.transaction || this.query;
    ...
    update(resourceConfig, id, attrs, options) {
        ...
        let query = options && options.transaction || this.query;
        return query(getTable(resourceConfig))
            .where(resourceConfig.idAttribute, moutLang.toString(id))
            .update(attrs)
    ...
    filterQuery(resourceConfig, params, options) {
        ...
        Object.keys(params).forEach(k => {
            let v = params[k];
            if (!moutArray.contains(reserved, k)) {
                if (moutLang.isObject(v)) {
                    params.where[k] = v;
                } else {
                    params.where[k] = {
                        '==': v
        ...
        if (!moutLang.isEmpty(params.where)) {
            moutObj.forOwn(params.where, (criteria, field) => {
                if (!moutLang.isObject(criteria)) {
                    params.where[field] = {
                        '==': criteria
                    }
                }

                if (moutArray.contains(field, '.')) {
                    if (moutArray.contains(field, ',')) {
        ...
        if (params.orderBy) {
            if (moutLang.isString(params.orderBy)) {
                params.orderBy = [
                    [params.orderBy, 'asc']
                ];
            }
            for (var i = 0; i < params.orderBy.length; i++) {
                if (moutLang.isString(params.orderBy[i])) {
